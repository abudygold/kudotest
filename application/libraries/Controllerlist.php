...
class ControllerList {
	...
	private function setControllers() {
		...
		foreach(glob(APPPATH . 'controllers/*') as $k=>$controller) {
			
			/*-- Added strpos by Abdullah (abudy.gold@yahoo.com) --*/
			$find_string = 'v1';
			$check_string = strpos($controller, $find_string);
			...
			if ($check_string === false) {
				...
				if(is_dir($controller)) {
					...
					if(is_array($aMethods)){
						foreach($aMethods as $method) {
							if(
								$method != '__construct' && $method != 'get_instance' && $method != $controllername && 
								$method != '_get_csrf_nonce' && $method != '_valid_csrf_nonce' && $method != '_render_page' // Added by Abdullah
							) {
								$aUserMethods[] = $method;
							}
						}
