...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['pengguna'], $data, array('id' => $id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db->where('lupa_sandi_kode', $code);

		if ($this->db->count_all_results($this->tables['pengguna']) > 0)
		{
			$data = array(
			    'lupa_sandi_kode' => NULL,
			    'lupa_sandi_waktu' => NULL
			);

			$this->db->update($this->tables['pengguna'], $data, array('lupa_sandi_kode' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['pengguna'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['pengguna'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
										...
										->order_by("id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['pengguna']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$update = array(
		    'lupa_sandi_kode' => $key,
		    'lupa_sandi_waktu' => time()
		);

		$this->db->update($this->tables['pengguna'], $update, array($this->identity_column => $identity));
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('lupa_sandi_kode', $code)->pengguna()->row(); //pass the code to profile
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		elseif ( empty($additional_data['grup_id']) )
		{
		...
		//$query = $this->db->get_where($this->tables['grup'],array('nama' => $this->config->item('default_group', 'ion_auth')),1)->row();
		//if( !isset($query->id) && empty($groups) )
		$query = $this->db->get_where($this->tables['grup'], array('id' => $additional_data['grup_id']),1)->row();
		if( !isset($query->id) && empty($additional_data['grup_id']) )
		{
		...
		$data = array(
		    ...
		    'dibuat_tgl' => time(),
		    'aktif'     => ($manual_activation === false ? 1 : 0)
		);
		...
		$user_data = array_merge($this->_filter_data($this->tables['pengguna'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['pengguna'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, aktif, username, dibuat_tgl, terakhir_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['pengguna']);
	...
	public function recheck_session()
    {
        ...
        if($recheck!==0)
        {
            $last_login = $this->session->userdata('terakhir_login');
            if($last_login+$recheck < time())
            {
                $query = $this->db->select('id')
                    ->where(array($this->identity_column=>$this->session->userdata('identity'),'aktif'=>'1'))
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['pengguna']);
                if ($query->num_rows() === 1)
                {
                    $this->session->set_userdata('terakhir_login',time());
                }
                else
                {
                    ...
                    {
                        $this->session->unset_userdata( array($identity => '', 'id' => '', 'pengguna_id' => '') );
                    }
                    else
                    {
                        $this->session->unset_userdata( array($identity, 'id', 'pengguna_id') );
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['pengguna'].'.*',
			    $this->tables['pengguna'].'.id as id',
			    $this->tables['pengguna'].'.id as pengguna_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['grup_pengguna'],
				    $this->tables['grup_pengguna'].'.'.$this->join['pengguna'].'='.$this->tables['pengguna'].'.id',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['grup'], $this->tables['grup_pengguna'] . '.' . $this->join['grup'] . ' = ' . $this->tables['grup'] . '.id', 'inner');
				$this->db->where_in($this->tables['grup'] . '.nama', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['grup_pengguna'].'.'.$this->join['grup'], $group_ids);
			}
		}
		...
		$this->response = $this->db->get($this->tables['pengguna']);
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('pengguna_id');

		$this->limit(1);
		$this->order_by($this->tables['pengguna'].'.id', 'desc');
		$this->where($this->tables['pengguna'].'.id', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('pengguna_id');

		return $this->db->select($this->tables['grup_pengguna'].'.'.$this->join['grup'].' as id, '.$this->tables['grup'].'.nama, '.$this->tables['grup'].'.deskripsi, '.$this->tables['grup'].'.akses_menu')
		                ->where($this->tables['grup_pengguna'].'.'.$this->join['pengguna'], $id)
		                ->join($this->tables['grup'], $this->tables['grup_pengguna'].'.'.$this->join['grup'].'='.$this->tables['grup'].'.id')
		                ->get($this->tables['grup_pengguna']);
	}
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('pengguna_id');
		...
		{
			if ($this->db->insert($this->tables['grup_pengguna'], array( $this->join['grup'] => (float)$group_id, $this->join['pengguna'] => (float)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['grup_pengguna'], array($this->join['grup'] => (float)$group_id, $this->join['pengguna'] => (float)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['grup_pengguna'], array($this->join['pengguna'] => (float)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->db->get($this->tables['grup']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['grup'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['pengguna'], $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['pengguna'], $data, array('id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['pengguna'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['pengguna'], array('terakhir_login' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'dibuat_tgl'                => $user->dibuat_tgl,
		    'pengguna_id'              => $user->id, //everyone likes to overwrite id so we'll use user_id
		    'terakhir_login_lalu'       => $user->terakhir_login,
		    'terakhir_login'           => time(),
		);
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['pengguna'], array('remember_kode' => $salt), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, email, terakhir_login')
		                  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_auth'))))
		                  ->where('remember_kode', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
				  ->where('aktif',1)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['pengguna']);
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['grup'], array('nama' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$data = array('nama'=>$group_name,'deskripsi'=>$group_description);

		// filter out any data passed that doesnt have a matching column in the groups table
		// and merge the set group data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['grup'], $additional_data), $data);

		$this->trigger_events('extra_group_set');

		// insert the new group
		$this->db->insert($this->tables['grup'], $data);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['grup'], array('nama' => $group_name))->row();
			...
			$data['nama'] = $group_name;
		}

		// restrict change of name of the admin group
        $group = $this->db->get_where($this->tables['grup'], array('id' => $group_id))->row();
        if($this->config->item('admin_group', 'ion_auth') === $group->nama && $group_name !== $group->nama)
        {
		...
		if (is_string($additional_data)) $additional_data = array('deskripsi' => $additional_data);
		...
		$this->db->update($this->tables['grup'], $data, array('id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->nama == $this->config->item('admin_group', 'ion_auth'))
		{
		...
		$this->db->delete($this->tables['grup_pengguna'], array($this->join['grup'] => $group_id));
		// remove the group itself
		$this->db->delete($this->tables['grup'], array('id' => $group_id));
