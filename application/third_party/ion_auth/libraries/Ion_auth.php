...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('aktif', 1)->pengguna()->row();  // changed to get_user_by_identity from email

			if ($user)
			{
				$data = array(
					'identity'		=> $user->{$this->config->item('identity', 'ion_auth')},
					'lupa_sandi_kode' => $user->lupa_sandi_kode
				);
	...
	public function forgotten_password_complete($code)
	{
		...
		$profile  = $this->where('lupa_sandi_kode', $code)->users()->row(); //pass the code to profile
